...
function CompareBar({ label, valueA, valueB, higher }: CompareBarProps) {
  ...
  return (
    <div className="py-3">
      <p className="text-[11px] uppercase tracking-wider text-white/25 font-semibold text-center mb-2">
        {label}
      </p>
      <div className="flex items-center gap-3">
        <span
          className={`text-sm font-bold w-20 text-right font-mono ${
            aWins ? "text-emerald-400" : "text-white/50"
          }`}
        >
          {typeof valueA === "number" && valueA > 1000
          ...
          <div className="flex-1 flex justify-end">
            <div
              className={`h-full rounded-l transition-all duration-700 ${
                aWins ? "bg-emerald-500/50" : "bg-white/10"
              }`}
              style={{ width: `${pctA}%` }}
            />
          </div>
          <div className="flex-1">
            <div
              className={`h-full rounded-r transition-all duration-700 ${
                bWins ? "bg-emerald-500/50" : "bg-white/10"
              }`}
              style={{ width: `${pctB}%` }}
            />
          </div>
        ...
        <span
          className={`text-sm font-bold w-20 font-mono ${
            bWins ? "text-emerald-400" : "text-white/50"
          }`}
        >
          {typeof valueB === "number" && valueB > 1000
      ...
      {movie ? (
        ...
        <div className="relative">
          <div className="relative">
            ...
            />
            {searching && (
              <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gold/40 animate-spin" />
            )}
          </div>
    ...
    <div className="pt-24 pb-20 px-6 md:px-10 lg:px-20 max-w-[1200px] mx-auto">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-white/35 max-w-md mx-auto">
          Pick two movies and compare them head to head, ratings, budget, cast,
          and more.
        </p>
      </div>
      ...
      {bothSelected && (
        <div className="glass-card rounded-2xl p-6 md:p-8 animate-slide-up">
          ...
          <div className="mt-6 pt-6 border-t border-white/[0.04]">
            <p className="text-[11px] uppercase tracking-wider text-white/25 font-semibold text-center mb-4">
              Genres
            </p>
                  ...
                  <div key={c.id} className="flex items-center gap-2 justify-end">
                    ...
                    <div className="w-7 h-7 rounded-full overflow-hidden bg-surface-3 flex-shrink-0">
                      {c.profile_path ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-[9px] text-white/15">
                          {c.name?.[0]}
                        </div>
                      )}
                    </div>
